    ...
    nextToken() {
        ...
        if (chars.isNum(this.currentChar)) {
            ...
            if (this.currentChar === '.') {
                ...
                this.advance();
                if (!chars.isNum(this.currentChar)) {
                    this.throw('NumberFormat', 'Expected at least one number after decimal point');
                }
                while (chars.isNum(this.currentChar)) {
                    result += this.currentChar;
                    this.advance();
        ...
        this.throw('InvalidCharacter', `Unrecognized char "${this.currentChar}"`);
    }

    /**
     * Throws an error of the specified type with position information included.
